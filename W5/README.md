...
For the final week, we are working with rotated images.

<b> To use clustering: </b> <br>

```

python clusters.py [-p path]  [dm desc_method] [-cs color_space] [-cn cluster_number] 

Arguments: Path of the museum images.   
Default: os.getcwd()


desc_method, -dm: Which descriptors are going to be used? Should be a color or texture
                  descriptor.   
Default: "3d"
Arguments: At least one from [1d, 3d, DCT, LBP]


color_space, -cs:  Name of the color space we want to use.
Default: "RGB"
Arguments: RGB, HSB, LAB, YCRCB 


cluster_number, -cn:  Number of clusters.
Default: 10
```

<b> To use the main functionalities: </b> <br>
...
keypoint_size, -kp:   Keypoint size for local descriptors. If descriptor method is not a local descriptor, gets ignored.                 
Default: 1000


match_threshold, -mt:   Match threshold for local descriptors. If given query set image doesn't have a number of matches 
                        higher than the threshold for any museum image, it is labeled as [-1]. If descriptor method is 
                        not a local descriptor, gets ignored.                 
Default: 50
...
Default: "ORB"
Arguments: At least one from [1d, 3d, DCT, LBP, text, ORB, SIFT, DOG, HP]
